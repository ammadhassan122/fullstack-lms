  ...
  return (
    <Link href={linkHref} className="group block">
      <div className="relative rounded-2xl bg-zinc-900/50 border border-zinc-800 overflow-hidden hover:border-zinc-700 transition-all duration-300 hover:shadow-lg hover:shadow-violet-500/5">
        ...
        <div
          className={`h-36 pt-5 bg-linear-to-br ${styles.gradient} flex items-center justify-center relative overflow-hidden`}
        >
          {thumbnail?.asset?.url ? (
            <Image
            ...
            <div className="flex items-center justify-center w-full h-full text-white/20 py-4">
          ...
          {isCompleted ? (
            <div className="absolute top-3 right-3 flex items-center gap-2 px-3 py-1 rounded-md text-sm font-semibold bg-emerald-500/90 text-white">
              <CheckCircle2 className="w-5 h-5" />
              Completed
            </div>
          ) : (
            <div
              className={`absolute top-3 right-3 px-3 py-1 rounded-md text-sm font-semibold uppercase tracking-wide ${styles.badge}`}
            >
              {displayTier}
            </div>
          )}
